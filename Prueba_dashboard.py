...
st.checkbox("Aplicar filtros?", key="disabled")
...
chart4= alt.Chart(empleados).mark_point(filled=True).encode(alt.Y('average_work_hours', scale=alt.Scale(domain=[3500, 5200])), alt.X('performance_score')).properties(title='Relacion horas trabajadas salario')
